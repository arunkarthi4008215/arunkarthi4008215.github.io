(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["app-pages-masters-location-master-location-master-module"],{

/***/ "7E5n":
/*!*********************************************************************************!*\
  !*** ./src/app/pages/masters/location-master/location-master-routing.module.ts ***!
  \*********************************************************************************/
/*! exports provided: LocationMasterRoutingModule */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "LocationMasterRoutingModule", function() { return LocationMasterRoutingModule; });
/* harmony import */ var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! tslib */ "mrSG");
/* harmony import */ var _angular_core__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @angular/core */ "fXoL");
/* harmony import */ var _angular_router__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @angular/router */ "tyNb");
/* harmony import */ var _location_master_component__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./location-master.component */ "COAU");




const routes = [
    { path: '', component: _location_master_component__WEBPACK_IMPORTED_MODULE_3__["LocationMasterComponent"] }
];
let LocationMasterRoutingModule = class LocationMasterRoutingModule {
};
LocationMasterRoutingModule = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["NgModule"])({
        imports: [_angular_router__WEBPACK_IMPORTED_MODULE_2__["RouterModule"].forChild(routes)],
        exports: [_angular_router__WEBPACK_IMPORTED_MODULE_2__["RouterModule"]]
    })
], LocationMasterRoutingModule);



/***/ }),

/***/ "COAU":
/*!****************************************************************************!*\
  !*** ./src/app/pages/masters/location-master/location-master.component.ts ***!
  \****************************************************************************/
/*! exports provided: LocationMasterComponent */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "LocationMasterComponent", function() { return LocationMasterComponent; });
/* harmony import */ var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! tslib */ "mrSG");
/* harmony import */ var _raw_loader_location_master_component_html__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! raw-loader!./location-master.component.html */ "CYTY");
/* harmony import */ var _location_master_component_css__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./location-master.component.css */ "CpBY");
/* harmony import */ var _angular_core__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! @angular/core */ "fXoL");
/* harmony import */ var _angular_forms__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! @angular/forms */ "3Pt+");
/* harmony import */ var services_utility_service__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! services/utility.service */ "2qeI");
/* harmony import */ var app_services_data_table_service__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! app/services/data-table.service */ "vO8+");
/* harmony import */ var rxjs__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! rxjs */ "qCKp");
/* harmony import */ var angular_datatables__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! angular-datatables */ "njyG");
/* harmony import */ var _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! @ng-bootstrap/ng-bootstrap */ "1kSV");
/* harmony import */ var _location_master_service__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ./location-master.service */ "Kn2c");
/* harmony import */ var sweetalert2__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! sweetalert2 */ "PSD3");
/* harmony import */ var sweetalert2__WEBPACK_IMPORTED_MODULE_11___default = /*#__PURE__*/__webpack_require__.n(sweetalert2__WEBPACK_IMPORTED_MODULE_11__);












let LocationMasterComponent = class LocationMasterComponent {
    constructor(utility, dataTable, ngmodal, formBuilder, locationService) {
        this.utility = utility;
        this.dataTable = dataTable;
        this.ngmodal = ngmodal;
        this.formBuilder = formBuilder;
        this.locationService = locationService;
        this.dtOptions = {};
        this.dtTrigger = new rxjs__WEBPACK_IMPORTED_MODULE_7__["Subject"]();
        this.isFilterModalOpen = false;
        this.locationForm = this.formBuilder.group({
            locationCode: ['', _angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required],
            locationName: ['', _angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required],
            city: ['', _angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required],
            state: ['', _angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required],
            country: ['', _angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required],
            address1: [''],
            address2: [''],
            pincode: ['', _angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required],
            contactPersonName: ['', _angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required],
            contactPersonNo: ['', _angular_forms__WEBPACK_IMPORTED_MODULE_4__["Validators"].required]
        });
    }
    ngOnInit() {
        this.dtOptions = this.dataTable.getDataTableOptions(false, true, 'Stock Receive List');
        this.getLocationList();
    }
    get loctionform() {
        return this.locationForm.controls;
    }
    getLocationList() {
        this.loader = true;
        this.nodata = true;
        this.locationService.getLocationList().subscribe(response => {
            const locationlist = response.data || {};
            this.loader = false;
            if (locationlist.length > 0) {
                setTimeout(() => (this.nodata = false), 500);
                this.locationList = locationlist;
            }
        }, error => { this.loader = false, this.nodata = true, this.utility.toast('top', 'right', '!Response Error', 'warning'); });
    }
    getLocationCode() {
        this.locationService.getLocationCode().subscribe(resp => {
            const locationCode = resp.data[0] || {};
            this.locationForm.controls['locationCode'].setValue(locationCode.Location_Code);
        }, error => {
            console.log(error);
        });
    }
    addLocation(content) {
        this.getLocationCode();
        const addDialog = this.ngmodal.open(content, { size: 'xl', backdropClass: 'bg-secondary' });
        this.popupTitle = 'Addbranch';
        this.edit = false;
        addDialog.result.then((result) => {
            this.getLocationList();
        }).catch((error) => {
        });
    }
    editLocation(content, id) {
        const editDialog = this.ngmodal.open(content, { size: 'md', backdropClass: 'bg-secondary' });
        this.popupTitle = 'Editbranch';
        this.edit = true;
        this.locationId = id;
        this.locationService.getLocationData(id).subscribe(resp => {
            const locationData = resp.data || {};
            this.locationForm.controls['locationCode'].setValue(locationData[0].locationCode);
            this.locationForm.controls['locationName'].setValue(locationData[0].locationName);
            this.locationForm.controls['pincode'].setValue(locationData[0].pincode);
            this.locationForm.controls['address1'].setValue(locationData[0].address_line1);
            this.locationForm.controls['address1'].setValue(locationData[0].address_line2);
            this.locationForm.controls['city'].setValue(locationData[0].city);
            this.locationForm.controls['state'].setValue(locationData[0].state);
            this.locationForm.controls['country'].setValue(locationData[0].country);
            this.locationForm.controls['contactPersonName'].setValue(locationData[0].locationContactPerson);
            this.locationForm.controls['contactPersonNo'].setValue(locationData[0].locationContactPersonNo);
        }, error => {
            console.log(error);
        });
        editDialog.result.then((result) => {
            this.getLocationList();
        }).catch((error) => {
        });
    }
    onSubmit() {
        this.submitted = true;
        // stop here if form is invalid
        if (this.locationForm.invalid) {
            return;
        }
        else {
            if (this.locationId) {
                this.locationForm.value.locationId = this.locationId;
                this.locationService.updateLocationMaster(this.locationForm.value).subscribe(resp => {
                    const branchCode = resp.data || {};
                    if (resp.success) {
                        this.utility.toast('top', 'right', 'Updated Successfully', 'success');
                        this.locationForm.reset();
                        this.ngmodal.dismissAll();
                    }
                    else if (resp.status == 404) {
                        this.utility.toast('top', 'right', 'Failed to Update', 'warning');
                    }
                }, error => {
                    this.utility.toast('top', 'right', '!Server Error', 'warning');
                });
            }
            else {
                this.locationService.saveLocationMaster(this.locationForm.value).subscribe(resp => {
                    const branchCode = resp.data || {};
                    if (resp.success) {
                        this.utility.toast('top', 'right', 'Saved Successfully', 'success');
                        this.ngmodal.dismissAll();
                    }
                    else if (resp.status == 404) {
                        this.utility.toast('top', 'right', 'Failed to Save', 'warning');
                    }
                }, error => {
                    this.utility.toast('top', 'right', '!Server Error', 'warning');
                });
            }
        }
    }
    deleteLocation() {
        sweetalert2__WEBPACK_IMPORTED_MODULE_11___default.a.fire({
            title: 'Are you sure?',
            text: 'This process is irreversible.',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Yes, go ahead.',
            cancelButtonText: 'No, let me think',
        }).then((result) => {
            if (result.value) {
                this.locationService.deleteLocationMaster(this.locationId).subscribe(resp => {
                    if (resp.status == 200) {
                        sweetalert2__WEBPACK_IMPORTED_MODULE_11___default.a.fire('Removed!', 'Location removed successfully.', 'success');
                        this.ngmodal.dismissAll();
                        this.getLocationList();
                    }
                    else {
                        sweetalert2__WEBPACK_IMPORTED_MODULE_11___default.a.fire('Cancelled', 'Location still in our database.)', 'error');
                        this.ngmodal.dismissAll();
                    }
                });
            }
            else if (result.dismiss === sweetalert2__WEBPACK_IMPORTED_MODULE_11___default.a.DismissReason.cancel) {
                this.ngmodal.dismissAll();
                sweetalert2__WEBPACK_IMPORTED_MODULE_11___default.a.fire('Cancelled', 'Location still in our database.)', 'error');
            }
        });
    }
};
LocationMasterComponent.ctorParameters = () => [
    { type: services_utility_service__WEBPACK_IMPORTED_MODULE_5__["UtilityService"] },
    { type: app_services_data_table_service__WEBPACK_IMPORTED_MODULE_6__["DataTableService"] },
    { type: _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_9__["NgbModal"] },
    { type: _angular_forms__WEBPACK_IMPORTED_MODULE_4__["FormBuilder"] },
    { type: _location_master_service__WEBPACK_IMPORTED_MODULE_10__["LocationService"] }
];
LocationMasterComponent.propDecorators = {
    dtElement: [{ type: _angular_core__WEBPACK_IMPORTED_MODULE_3__["ViewChild"], args: [angular_datatables__WEBPACK_IMPORTED_MODULE_8__["DataTableDirective"], { static: false },] }]
};
LocationMasterComponent = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_3__["Component"])({
        selector: 'app-location-master',
        template: _raw_loader_location_master_component_html__WEBPACK_IMPORTED_MODULE_1__["default"],
        styles: [_location_master_component_css__WEBPACK_IMPORTED_MODULE_2__["default"]]
    }),
    Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"])("design:paramtypes", [services_utility_service__WEBPACK_IMPORTED_MODULE_5__["UtilityService"],
        app_services_data_table_service__WEBPACK_IMPORTED_MODULE_6__["DataTableService"],
        _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_9__["NgbModal"],
        _angular_forms__WEBPACK_IMPORTED_MODULE_4__["FormBuilder"],
        _location_master_service__WEBPACK_IMPORTED_MODULE_10__["LocationService"]])
], LocationMasterComponent);



/***/ }),

/***/ "CYTY":
/*!********************************************************************************************************************!*\
  !*** ./node_modules/raw-loader/dist/cjs.js!./src/app/pages/masters/location-master/location-master.component.html ***!
  \********************************************************************************************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony default export */ __webpack_exports__["default"] = ("<div *ngIf=\"loader\"  class=\"row\">\r\n    <div class=\"col-md-12\">\r\n      <span>\r\n        <app-loader></app-loader>\r\n        </span>\r\n    </div>\r\n    </div>\r\n    <div *ngIf=\"!loader\" class=\"row\">\r\n      <div  class=\"col-md-12\">\r\n        <div class=\"card\">\r\n          <div class=\"card-header\">\r\n            <div class=\"d-flex\" style=\"justify-content: space-between;\">\r\n              <div class=\"mr-auto p-2\">\r\n                <h5>\r\n                  <span><a href=\"#/app/settings\">Settings</a></span>\r\n                  <span class=\"mx-2\"> <i class=\"fa fa-angle-right\"></i></span>\r\n                  <span>Location</span>\r\n                </h5>\r\n              </div>\r\n              <div class=\"justify-content-md-end flex-col\">\r\n              <button type=\"button\" class=\"btn btn-success\" (click)=\"addLocation(content)\">\r\n                Add\r\n                <i class=\"fas fa-plus\"></i>\r\n              </button>\r\n              <button type=\"button\" class=\"btn btn-outline-danger pt-1\"\r\n                (click)=\"navigateToBack()\">\r\n                <span>\r\n                  <svg xmlns=\"http://www.w3.org/2000/svg\" width=\"16\" height=\"16\" fill=\"currentColor\"\r\n                    class=\"bi bi-arrow-return-left\" viewBox=\"0 0 16 16\">\r\n                    <path fill-rule=\"evenodd\"\r\n                      d=\"M14.5 1.5a.5.5 0 0 1 .5.5v4.8a2.5 2.5 0 0 1-2.5 2.5H2.707l3.347 3.346a.5.5 0 0 1-.708.708l-4.2-4.2a.5.5 0 0 1 0-.708l4-4a.5.5 0 1 1 .708.708L2.707 8.3H12.5A1.5 1.5 0 0 0 14 6.8V2a.5.5 0 0 1 .5-.5z\" />\r\n                  </svg>\r\n                </span>\r\n              </button>\r\n              </div>\r\n            </div>\r\n          </div>\r\n        </div>\r\n        <div class=\"card\">\r\n          <div class=\"card-body\">\r\n            <div>\r\n              <div class=\"table-responsive border p-2 mt-4\">\r\n               <table class=\"table table-bordered\" datatable [dtOptions]=\"dtOptions\" datatable class=\"row-border hover\">\r\n                <thead>\r\n                    <tr>\r\n                      <th>Location Code</th>\r\n                      <th>Location Name</th>\r\n                      <th>City</th>\r\n                      <th>State</th>\r\n                      <th>Country</th>\r\n                    </tr>\r\n                  </thead> \r\n                  <tbody>\r\n                    <tr *ngFor=\"let location of locationList\" (click)=\"editLocation(content,location.Id)\">\r\n                      <td>\r\n                        {{location.locationCode}}\r\n                      </td>\r\n                      <td>\r\n                          {{location.locationName}}\r\n                      </td>\r\n                      <td>\r\n                        {{location.city}}\r\n                      </td>\r\n                      <td>\r\n                        {{location.state}}\r\n                      </td>\r\n                      <td>\r\n                        {{location.country}}\r\n                      </td>\r\n                    </tr>\r\n                  </tbody>\r\n                </table>\r\n              </div>\r\n              <div>\r\n              </div>\r\n            </div>\r\n          </div>\r\n        </div>\r\n    \r\n      </div>\r\n    </div>\r\n  \r\n    <div class=\"row\" *ngIf=\"nodata\">\r\n      <div class=\"col-12\">\r\n        <div class=\"empty-state\">\r\n          <div class=\"empty-state__content\">\r\n            <div class=\"empty-state__icon\">\r\n              <img src=\"data:image/png;base64,\" alt=\"\">\r\n            </div>\r\n            <div class=\"empty-state__message\">No records has been added yet.</div>\r\n            <div class=\"empty-state__help\">\r\n              <span>Add a new record by simply clicking the </span>\r\n              <span class=\"btn btn-success\" (click)=\"addLocation(content)\">Add Location<i class=\"fas fa-plus\"></i></span> \r\n              <span>Button.</span>\r\n            </div>\r\n          </div>\r\n        </div>\r\n      </div>\r\n    </div>\r\n    <div>\r\n    </div>\r\n\r\n<!-- ******************************************************************************************* -->\r\n<!-- *                                                                                          *-->\r\n<!-- *                  Location Master PopUp                                                   *-->\r\n<!-- *                                                                                          *-->\r\n<!-- ******************************************************************************************* -->\r\n\r\n\r\n<ng-template #content let-modal>\r\n    <div class=\"modal-content bg-light\">\r\n        <div class=\"modal-header\">\r\n          <h4 *ngIf=\"!edit\" class=\"modal-title\" id=\"modal-basic-title\">Add Location</h4>\r\n          <h4 *ngIf=\"edit\" class=\"modal-title\" id=\"modal-basic-title\">Edit Location</h4>\r\n          <button type=\"button\" class=\"btn-close\" aria-label=\"Close\" (click)=\"modal.close('Close click')\"></button>\r\n        </div>\r\n        <div class=\"modal-body\">\r\n          <form class=\"form\" [formGroup]=\"locationForm\" (ngSubmit)=\"onSubmit()\">\r\n            <div class=\"form-group\">\r\n                <label>*Location Code</label>\r\n                <input type=\"text\" class=\"form-control\" disabled name=\"locationCode\" formControlName=\"locationCode\">\r\n                <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.locationCode.touched || submitted) && locationForm.controls.locationCode.errors?.required\">\r\n                    Location Code is required\r\n                </span>\r\n            </div>\r\n            <div class=\"form-group\">\r\n                <label>*Location Name</label>\r\n                <input type=\"text\" class=\"form-control\" name=\"locationName\" formControlName=\"locationName\">\r\n                <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.locationName.touched || submitted) && locationForm.controls.locationName.errors?.required\">\r\n                    Location Name is required\r\n                </span>\r\n            </div>\r\n            <div class=\"form-group\">\r\n                <label>*City</label>\r\n                <input type=\"text\" class=\"form-control\" name=\"city\" formControlName=\"city\">\r\n                <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.city.touched || submitted) && locationForm.controls.city.errors?.required\">\r\n                    City is required\r\n                </span>\r\n            </div>\r\n            <div class=\"form-group\">\r\n                <label>*State</label>\r\n                <input type=\"text\" class=\"form-control\" name=\"state\" formControlName=\"state\">\r\n                <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.state.touched || submitted) && locationForm.controls.state.errors?.required\">\r\n                    State is required\r\n                </span>\r\n            </div>\r\n            <div class=\"form-group\">\r\n                <label>*Country</label>\r\n                <input type=\"text\" class=\"form-control\" name=\"country\" formControlName=\"country\">\r\n                <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.country.touched || submitted) && locationForm.controls.country.errors?.required\">\r\n                    Country is required\r\n                </span>\r\n            </div>\r\n            <div class=\"form-group\">\r\n                <label>Address-1</label>\r\n                <input type=\"text\" class=\"form-control\" name=\"address1\" formControlName=\"address1\">\r\n                <!-- <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.country.touched || submitted) && locationForm.controls.country.errors?.required\">\r\n                    Country is required\r\n                </span> -->\r\n            </div>\r\n            <div class=\"form-group\">\r\n                <label>Address-2</label>\r\n                <input type=\"text\" class=\"form-control\" name=\"address2\" formControlName=\"address2\">\r\n                <!-- <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.address2.touched || submitted) && locationForm.controls.address2.errors?.required\">\r\n                    Country is required\r\n                </span> -->\r\n            </div>\r\n            <div class=\"form-group\">\r\n                <label>Pin Code</label>\r\n                <input type=\"text\" class=\"form-control\" name=\"pincode\" formControlName=\"pincode\" maxlength=\"6\">\r\n                <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.pincode.touched || submitted) && locationForm.controls.pincode.errors?.required\">\r\n                    Pin is required\r\n                </span>\r\n            </div>\r\n            <div class=\"form-group\">\r\n                <label>Location Contact Person Name</label>\r\n                <input type=\"text\" class=\"form-control\" name=\"contactPersonName\" formControlName=\"contactPersonName\">\r\n                <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.contactPersonName.touched || submitted) && locationForm.controls.contactPersonName.errors?.required\">\r\n                    Location Contact Person Name is required\r\n                </span>\r\n            </div>\r\n            <div class=\"form-group\">\r\n                <label>Location Contact Person Number</label>\r\n                <input type=\"text\" class=\"form-control\" name=\"contactPersonNo\" maxlength=\"17\" formControlName=\"contactPersonNo\">\r\n                <span class=\"text-danger\"\r\n                    *ngIf=\"(locationForm.controls.contactPersonNo.touched || submitted) && locationForm.controls.contactPersonNo.errors?.required\">\r\n                    Location Contact Person Number is required\r\n                </span>\r\n            </div>\r\n            <div class=\"modal-footer\">\r\n                <span *ngIf=\"!edit\">\r\n                <button type=\"submit\" class=\"btn btn-success\">Save</button>\r\n                </span>\r\n                <span *ngIf=\"edit\">\r\n                <button type=\"submit\" class=\"btn btn-success\">Update</button>\r\n                <button type=\"button\" (click)=\"deleteLocation(locationId)\" class=\"btn btn-danger\">Delete</button>\r\n                </span>\r\n                <button type=\"button\" (click)=\"modal.close('Close click')\" class=\"btn btn-warning\">Close</button>\r\n            </div>\r\n        </form>\r\n        </div>\r\n      \r\n        </div>\r\n  </ng-template>");

/***/ }),

/***/ "CpBY":
/*!*****************************************************************************!*\
  !*** ./src/app/pages/masters/location-master/location-master.component.css ***!
  \*****************************************************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony default export */ __webpack_exports__["default"] = ("\n/*# sourceMappingURL=data:application/json;base64,eyJ2ZXJzaW9uIjozLCJzb3VyY2VzIjpbXSwibmFtZXMiOltdLCJtYXBwaW5ncyI6IiIsImZpbGUiOiJsb2NhdGlvbi1tYXN0ZXIuY29tcG9uZW50LmNzcyJ9 */");

/***/ }),

/***/ "Kn2c":
/*!**************************************************************************!*\
  !*** ./src/app/pages/masters/location-master/location-master.service.ts ***!
  \**************************************************************************/
/*! exports provided: LocationService */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "LocationService", function() { return LocationService; });
/* harmony import */ var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! tslib */ "mrSG");
/* harmony import */ var _angular_common_http__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @angular/common/http */ "tk/3");
/* harmony import */ var _angular_core__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @angular/core */ "fXoL");
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! rxjs/operators */ "kU1M");
/* harmony import */ var environments_environment__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! environments/environment */ "AytR");





let LocationService = class LocationService {
    constructor(http) {
        this.http = http;
        this.BaseAdminApiURL = environments_environment__WEBPACK_IMPORTED_MODULE_4__["environment"].apiURL;
        this.BaseAdminLocalApiURL = environments_environment__WEBPACK_IMPORTED_MODULE_4__["environment"].apiURL;
    }
    saveLocationMaster(branchObj) {
        const body = branchObj;
        return this.http.post(this.BaseAdminApiURL + 'locationapi/insertLocation', body).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(response => {
            return response;
        }, (error) => console.error(error, 'api-Handler')));
    }
    getLocationList() {
        return this.http.get(this.BaseAdminApiURL + 'locationapi/getLocationList').pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(response => {
            return response;
        }, (error) => console.error(error, 'api-Handler')));
    }
    getLocationData(locationId) {
        const params = { locationId: locationId };
        const encodedUrl = encodeURI(this.BaseAdminApiURL + 'locationapi/getLocationDetails');
        return this.http.get(encodedUrl, { params }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(response => {
            return response;
        }, (error) => console.error(error, 'api-Handler')));
    }
    updateLocationMaster(branchObj) {
        const body = branchObj;
        return this.http.post(this.BaseAdminApiURL + 'locationapi/updateLocation', body).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(response => {
            return response;
        }, (error) => console.error(error, 'api-Handler')));
    }
    deleteLocationMaster(locationId) {
        const body = { locationId };
        return this.http.post(this.BaseAdminApiURL + 'locationapi/deleteLocation', body).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(response => {
            return response;
        }, (error) => console.error(error, 'api-Handler')));
    }
    getLocationCode() {
        return this.http.get(this.BaseAdminApiURL + 'locationapi/getLocationCode').pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(response => {
            return response;
        }, (error) => console.error(error, 'api-Handler')));
    }
};
LocationService.ctorParameters = () => [
    { type: _angular_common_http__WEBPACK_IMPORTED_MODULE_1__["HttpClient"] }
];
LocationService = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_2__["Injectable"])({
        providedIn: 'root'
    }),
    Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__metadata"])("design:paramtypes", [_angular_common_http__WEBPACK_IMPORTED_MODULE_1__["HttpClient"]])
], LocationService);



/***/ }),

/***/ "ljFg":
/*!*************************************************************************!*\
  !*** ./src/app/pages/masters/location-master/location-master.module.ts ***!
  \*************************************************************************/
/*! exports provided: LocationMasterModule */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "LocationMasterModule", function() { return LocationMasterModule; });
/* harmony import */ var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! tslib */ "mrSG");
/* harmony import */ var _angular_core__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @angular/core */ "fXoL");
/* harmony import */ var _angular_common__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @angular/common */ "ofXK");
/* harmony import */ var _location_master_routing_module__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./location-master-routing.module */ "7E5n");
/* harmony import */ var _location_master_component__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./location-master.component */ "COAU");
/* harmony import */ var _angular_forms__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! @angular/forms */ "3Pt+");
/* harmony import */ var _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! @ng-bootstrap/ng-bootstrap */ "1kSV");
/* harmony import */ var _angular_common_http__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! @angular/common/http */ "tk/3");
/* harmony import */ var angular_datatables__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! angular-datatables */ "njyG");









let LocationMasterModule = class LocationMasterModule {
};
LocationMasterModule = Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"])([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["NgModule"])({
        declarations: [_location_master_component__WEBPACK_IMPORTED_MODULE_4__["LocationMasterComponent"]],
        imports: [
            _angular_common__WEBPACK_IMPORTED_MODULE_2__["CommonModule"],
            _angular_common_http__WEBPACK_IMPORTED_MODULE_7__["HttpClientModule"],
            _angular_forms__WEBPACK_IMPORTED_MODULE_5__["FormsModule"],
            _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_6__["NgbModule"],
            angular_datatables__WEBPACK_IMPORTED_MODULE_8__["DataTablesModule"],
            _angular_forms__WEBPACK_IMPORTED_MODULE_5__["ReactiveFormsModule"],
            _location_master_routing_module__WEBPACK_IMPORTED_MODULE_3__["LocationMasterRoutingModule"]
        ]
    })
], LocationMasterModule);



/***/ })

}]);
//# sourceMappingURL=app-pages-masters-location-master-location-master-module-es2015.js.map